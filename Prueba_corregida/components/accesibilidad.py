...
def _css_resaltar_focus() -> str:
    """CSS para resaltar foco de teclado"""
    return """
    <style>
      :root {
        --a11y-ring: #ffbf47;
        --a11y-ring2: rgba(255,191,71,.35);
      }
      .kbd-nav :focus:not(:focus-visible) {
      ...
      .kbd-nav :focus-visible {
        ...
        box-shadow: 0 0 0 4px var(--a11y-ring2) !important;
        border-radius: 8px !important;
      ...
      (function(){
        ...
        d.addEventListener('keydown', e => {
          if (e.key === 'Tab' || e.key.startsWith('Arrow')) {
            d.documentElement.classList.add('kbd-nav');
          }
        }, {passive:true});
        d.addEventListener('mousedown', () => d.documentElement.classList.remove('kbd-nav'), {passive:true});
      })();
    </script>
    """
...
def aplicar_accesibilidad():
    ...
    # Aplicar modo oscuro
    if st.session_state.get("a11y_modo_oscuro", False):
        _inject(_css_modo_oscuro())
    
    # Aplicar alto contraste (solo si no está en modo oscuro)
    if st.session_state.get("a11y_contraste", False) and not st.session_state.get("a11y_modo_oscuro", False):
        _inject(_css_contraste_alto())
    ...
    # Aplicar modo enfoque
    if st.session_state.get("a11y_enfoque", False):
        _inject(_css_modo_enfoque())
...
def _mostrar_contenido_panel_accesibilidad():
    ...
    # Alto contraste (solo en modo claro)
    if not modo_oscuro:
        alto_contraste = st.checkbox(
            "⚡ Alto contraste (modo blanco)",
    ...
    with c2:
        enfoque = st.checkbox(
            "Modo enfoque",
            value=st.session_state.get("a11y_enfoque", False),
            key="a11y_enfoque_cb",
        )
        st.session_state["a11y_enfoque"] = enfoque
        _guardar_si_cambio("a11y_enfoque", enfoque, "a11y_enfoque_previo")

...
def _inject_tts_hover():
    ...
    (function() {{
        ...
        function getText(element) {{
            ...
            // Radio buttons de Streamlit
            const radio = element.closest('[data-testid="stRadio"]');
            if (radio) {{
                const label = radio.textContent ? radio.textContent.trim() : '';
                const selected = radio.querySelector('input[type="radio"]:checked');
                return label ? 'Opción ' + label + (selected ? ', seleccionada' : '') : null;
            }}
            ...
            // GRÁFICAS E IMÁGENES - Detección mejorada con descripciones completas
            if (tag === 'IMG') {{
                // Verificar si es una imagen de gráfico (grande, probablemente matplotlib)
                const esGrafico = (element.width && element.width > 200 && element.height && element.height > 150) ||
                                  element.src.includes('data:image') ||
                                  element.closest('[data-testid*="stImage"], [class*="stImage"]');
                
                if (!esGrafico) {{
                    // Si no es un gráfico, continuar con el flujo normal
                    return null;
                }}
                
                // Buscar título o descripción cercana
                const alt = element.alt || element.getAttribute('aria-label') || element.title;
                if (alt && alt.length > 0 && alt !== '0' && alt !== 'graph') {{
                    return 'Gráfico: ' + alt;
                }}
                
                // Función para construir descripción completa
                function construirDescripcionGrafico(titulo, contexto, elemento) {{
                    let descripcion = '';
                    
                    // Si no hay título, intentar identificar el gráfico por contexto
                    if (!titulo || titulo.length === 0) {{
                        // Buscar en el contexto de la página
                        const pageContext = document.body.textContent || '';
                        const imgRect = elemento.getBoundingClientRect();
                        
                        // Identificar por posición y contexto
                        if (pageContext.includes('Distribución de Calificaciones') && imgRect.top < window.innerHeight * 0.5) {{
                            titulo = 'Distribución de Calificaciones';
                        }} else if (pageContext.includes('Análisis de Asistencia')) {{
                            titulo = 'Análisis de Asistencia';
                        }} else if (pageContext.includes('Tendencia por Unidades')) {{
                            titulo = 'Tendencia por Unidades';
                        }} else {{
                            titulo = 'Gráfico de datos';
                        }}
                    }}
                    
                    descripcion = 'Gráfico: ' + titulo;
                    ...
                    // Detectar tipo de gráfico por el título y contexto
                    const tituloLower = titulo.toLowerCase();
                    
                    // DASHBOARD - Descripciones específicas mejoradas y detalladas
                    ...
                    }} else if (tituloLower.includes('asistencia') && tituloLower.includes('promedio')) {{
                        descripcion += ' Este gráfico de barras horizontales muestra el promedio de asistencia por materia. Las materias están ordenadas de mayor a menor asistencia de arriba hacia abajo. Cada barra muestra el porcentaje promedio de asistencia de esa materia. Esto te permite identificar rápidamente qué materias tienen problemas de asistencia y cuáles tienen buena asistencia.';
                    }} else if (tituloLower.includes('asistencia') && tituloLower.includes('grupo')) {{
                        descripcion += ' Este gráfico muestra la asistencia individual de los estudiantes de un grupo específico. Las barras horizontales muestran el porcentaje de asistencia de cada estudiante, ordenados de menor a mayor asistencia. Los estudiantes con asistencia menor al 80 por ciento aparecen en color rojo o diferente, indicando que requieren atención. Esto te permite identificar rápidamente a los estudiantes que tienen problemas de asistencia en el grupo seleccionado.';
                    ...
                    }} else if (tituloLower.includes('tendencia') && tituloLower.includes('calificaciones')) {{
                        descripcion += ' Este gráfico muestra la evolución de las calificaciones promedio a través de las unidades académicas. Las barras verticales representan cada unidad (U1, U2, U3), permitiendo comparar el rendimiento entre unidades y identificar tendencias en el desempeño académico a lo largo del semestre.';
                    }}
                    // ANÁLISIS DE CALIDAD
                    else if (tituloLower.includes('pareto')) {{
                        ...
                        descripcion += ' Este gráfico de dispersión muestra la relación entre dos variables numéricas. Cada punto representa una observación, permitiendo identificar correlaciones, tendencias o patrones entre las variables analizadas.';
                    ...
                    }} else if (tituloLower.includes('asistencia')) {{
                        descripcion += ' Este gráfico muestra información sobre la asistencia de los estudiantes, permitiendo identificar patrones y áreas que requieren atención.';
                    }} else if (tituloLower.includes('tendencia')) {{
                        descripcion += ' Este gráfico muestra las tendencias o promedios a lo largo del tiempo o por unidades académicas, facilitando la identificación de patrones y cambios en el rendimiento.';
                    }} else if (tituloLower.includes('calificaciones')) {{
                        descripcion += ' Este gráfico muestra información sobre las calificaciones de los estudiantes, permitiendo analizar el rendimiento académico.';
                    }}
                    
                    return descripcion;
                }}
                
                // Buscar título y contexto en elementos anteriores
                let tituloEncontrado = '';
                let contextoEncontrado = '';
                let prevSibling = element.previousElementSibling;
                
                for (let i = 0; i < 15 && prevSibling; i++) {{
                    const prevTag = prevSibling.tagName;
                    
                    // Buscar títulos
                    if (['H1','H2','H3','H4','H5','H6'].includes(prevTag)) {{
                        const titleText = prevSibling.textContent ? prevSibling.textContent.trim() : '';
                        if (titleText && titleText.length > 0 && !tituloEncontrado) {{
                            tituloEncontrado = titleText;
                        }}
                    }}
                    ...
                    if (prevSibling.querySelector) {{
                        ...
                        const subheader = prevSibling.querySelector('[class*="sub-header"], [class*="subheader"]');
                        if (subheader && !tituloEncontrado) {{
                            const subheaderText = subheader.textContent ? subheader.textContent.trim() : '';
                            if (subheaderText && subheaderText.length > 0) {{
                                tituloEncontrado = subheaderText;
                            }}
                        }}
                        
                        // Buscar captions o descripciones
                        const caption = prevSibling.querySelector('[class*="caption"], small, p');
                        if (caption && !contextoEncontrado) {{
                            const captionText = caption.textContent ? caption.textContent.trim() : '';
                            if (captionText && captionText.length > 10 && captionText.length < 200) {{
                                contextoEncontrado = captionText;
                            }}
                        }}
                    }}
                ...
                // Buscar en contenedores padre
                let container = element.parentElement;
                for (let i = 0; i < 6 && container; i++) {{
                    // Buscar título
                    if (!tituloEncontrado) {{
                        const titleInContainer = container.querySelector('h1, h2, h3, h4, h5, h6');
                        if (titleInContainer) {{
                            const titleText = titleInContainer.textContent ? titleInContainer.textContent.trim() : '';
                            if (titleText && titleText.length > 0) {{
                                tituloEncontrado = titleText;
                            }}
                        }}
                        
                        const subheader = container.querySelector('[class*="sub-header"], [class*="subheader"], .sub-header');
                        if (subheader && !tituloEncontrado) {{
                            const subheaderText = subheader.textContent ? subheader.textContent.trim() : '';
                            if (subheaderText && subheaderText.length > 0) {{
                                tituloEncontrado = subheaderText;
                    ...
                    // Buscar en elementos anteriores del contenedor
                    let prevInContainer = container.previousElementSibling;
                    for (let j = 0; j < 5 && prevInContainer && !tituloEncontrado; j++) {{
                        const prevTitle = prevInContainer.querySelector('h1, h2, h3, h4, h5, h6, [class*="sub-header"], [class*="subheader"]');
                        if (prevTitle) {{
                            const prevTitleText = prevTitle.textContent ? prevTitle.textContent.trim() : '';
                            if (prevTitleText && prevTitleText.length > 0) {{
                                tituloEncontrado = prevTitleText;
                            }}
                        }}
                        prevInContainer = prevInContainer.previousElementSibling;
                    }}
                    
                    container = container.parentElement;
                }}
                
                // Si encontramos un título, construir descripción completa
                if (tituloEncontrado && tituloEncontrado.length > 0) {{
                    const descripcion = construirDescripcionGrafico(tituloEncontrado, contextoEncontrado, element);
                    if (descripcion) {{
                        return descripcion;
                    }}
                }}
                
                // Buscar también en el DOM completo (último recurso)
                if (!tituloEncontrado) {{
                    // Buscar en toda la página por elementos cercanos
                    const allHeaders = document.querySelectorAll('h1, h2, h3, h4, h5, h6, [class*="sub-header"], [class*="subheader"]');
                    ...
                    for (let header of allHeaders) {{
                        ...
                        const distance = Math.abs(imgRect.top - headerRect.bottom);
                        
                        // Solo considerar headers que estén arriba de la imagen
                        if (headerRect.bottom < imgRect.top && distance < closestDistance && distance < 800) {{
                    ...
                    if (closestHeader) {{
                        const headerText = closestHeader.textContent ? closestHeader.textContent.trim() : '';
                        if (headerText && headerText.length > 0) {{
                            const descripcion = construirDescripcionGrafico(headerText, '', element);
                            if (descripcion) {{
                                return descripcion;
                            }}
                        }}
                    ...
                    // Si aún no encontramos título, usar contexto de la página
                    const pageText = document.body.textContent || '';
                    const imgRect2 = element.getBoundingClientRect();
                    const imgPosition = imgRect2.top / window.innerHeight;
                    
                    // Identificar gráfico por posición y contexto de la página
                    let tituloPorContexto = '';
                    if (pageText.includes('Distribución de Calificaciones') && imgPosition < 0.4) {{
                        tituloPorContexto = 'Distribución de Calificaciones';
                    }} else if (pageText.includes('Análisis de Asistencia') && imgPosition > 0.4 && imgPosition < 0.7) {{
                        tituloPorContexto = 'Análisis de Asistencia';
                    }} else if (pageText.includes('Tendencia por Unidades') && imgPosition > 0.4 && imgPosition < 0.7) {{
                        tituloPorContexto = 'Tendencia por Unidades';
                    }}
                    
                    if (tituloPorContexto) {{
                        const descripcion = construirDescripcionGrafico(tituloPorContexto, '', element);
                        if (descripcion) {{
                            return descripcion;
                        }}
                    }}
                ...
                // Si es una imagen grande (probablemente gráfico de matplotlib) pero no encontramos título
                if (element.width && element.width > 200 && element.height && element.height > 150) {{
                    // Intentar identificar por contexto de la página
                    const pageText = document.body.textContent || '';
                    if (pageText.includes('Distribución de Calificaciones')) {{
                        return construirDescripcionGrafico('Distribución de Calificaciones', '', element);
                    }} else if (pageText.includes('Análisis de Asistencia')) {{
                        return construirDescripcionGrafico('Análisis de Asistencia', '', element);
                    }} else if (pageText.includes('Tendencia por Unidades')) {{
                        return construirDescripcionGrafico('Tendencia por Unidades', '', element);
                    }}
                    return 'Gráfico o visualización de datos. Este gráfico muestra información estadística.';
                }}
                
                // Si no se encuentra título, describir como gráfico genérico
        ...
        function speak(text) {{
            ...
            setTimeout(function() {{
                ...
                utterance.onerror = function() {{ state.isSpeaking = false; }};
        ...
        function testMouseOver(e) {{
            console.log('%c🧪 TEST EVENTO CAPTURADO', 'color: red; font-size: 12px;', 
                       'Tag:', e.target.tagName, 
                       'Class:', e.target.className ? e.target.className.substring(0, 30) : 'sin clase');
        }}

...
def _css_resaltar_focus() -> str:
    """CSS para resaltar foco de teclado - MUY VISIBLE"""
    return """
    <style>
      ...
      /* Aplicar a TODOS los elementos enfocables - siempre visible */
      *:focus,
      *:focus-visible,
      button:focus,
      input:focus,
      ...
      @keyframes focusPulse {
        0%, 100% { box-shadow: 0 0 0 6px var(--a11y-focus-glow), 0 0 20px 4px var(--a11y-focus-glow); }
        50% { box-shadow: 0 0 0 8px var(--a11y-focus-glow), 0 0 30px 8px var(--a11y-focus-glow); }
      }
      ...
      *:focus-visible {
      ...
      /* Indicador visual adicional - flecha o borde */
      *:focus::before {
        content: '►' !important;
      ...
      /* Scrollbar al elemento enfocado */
      *:focus {
...
def _inject_tts_hover():
    ...
    # Generar un ID único basado en el tiempo para forzar reinicialización
    import time
    ...
    <script>
    (function() {{
        // Forzar reinicialización cada vez (el ID único hace que este script sea diferente)
        const scriptId = 'tts_hover_{unique_id}';
        ...
        const synth = window.speechSynthesis;
        if (!synth) {{
            console.error('[TTS Hover] Web Speech API no disponible en este navegador');
            return;
        }}
        ...
        const HOVER_DELAY = 400; // ms antes de empezar a leer
        const COOLDOWN = 1000; // ms antes de poder repetir el mismo texto
        
        // Función para obtener el texto más relevante de un elemento
        function getReadableText(element) {{
            if (!element || !element.tagName) return null;
            
            // Ignorar elementos del panel de accesibilidad para evitar bucles
            if (element.closest('[data-testid="stExpander"]') && 
                element.closest('[data-testid="stExpander"]').textContent.includes('Accesibilidad')) {{
                return null;
            }}
            ...
            if (tagName === 'IMG') {{
                ...
                if (container) {{
                    ...
                    if (prevTitle && ['H1','H2','H3','H4','H5','H6'].includes(prevTitle.tagName)) {{
                        return 'Gráfico: ' + prevTitle.textContent;
                    }}
                }}
                return 'Imagen o gráfico';
            }}
            
            // ===== CANVAS (gráficos interactivos) =====
            if (tagName === 'CANVAS') {{
                const container = element.closest('div');
                if (container) {{
                    const title = container.querySelector('h1, h2, h3, h4, h5, h6');
                    if (title) return 'Gráfico interactivo: ' + title.textContent;
                }}
            ...
            if (['H1', 'H2', 'H3', 'H4', 'H5', 'H6'].includes(tagName)) {{
                return 'Título: ' + element.textContent.trim();
            }}
            
            // ===== BOTONES =====
            if (tagName === 'BUTTON' || element.getAttribute('role') === 'button') {{
                const text = element.textContent?.trim() || element.getAttribute('aria-label') || 'sin nombre';
                return 'Botón: ' + text;
            }}
            
            // ===== ENLACES =====
            if (tagName === 'A') {{
                const text = element.textContent?.trim() || element.getAttribute('aria-label') || 'enlace';
                return 'Enlace: ' + text;
            }}
            ...
            if (tagName === 'INPUT') {{
                ...
                if (type === 'checkbox') {{
                    return 'Casilla ' + label + ', ' + (element.checked ? 'activada' : 'desactivada');
                }}
                if (type === 'radio') {{
                    return 'Opción ' + label + ', ' + (element.checked ? 'seleccionada' : 'no seleccionada');
                }}
                return 'Campo de ' + type + ' ' + label + (value ? ': ' + value : '');
            }}
            
            // ===== CHECKBOXES de Streamlit =====
            const stCheckbox = element.closest('[data-testid="stCheckbox"]');
            if (stCheckbox) {{
                const label = stCheckbox.textContent?.trim() || '';
                const input = stCheckbox.querySelector('input[type="checkbox"]');
                const checked = input?.checked ? 'activada' : 'desactivada';
                return 'Casilla ' + label + ', ' + checked;
            }}
            
            // ===== SELECT/DROPDOWN =====
            if (tagName === 'SELECT' || element.closest('[data-baseweb="select"]')) {{
                const container = element.closest('[data-baseweb="select"]') || element;
                const label = container.closest('div')?.querySelector('label')?.textContent || '';
                const value = container.textContent?.split('\\n')[0]?.trim() || '';
                return 'Menú ' + label + '. Valor: ' + value;
            }}
            
            // ===== MÉTRICAS de Streamlit =====
            const stMetric = element.closest('[data-testid="stMetric"]');
            if (stMetric) {{
                const labelEl = stMetric.querySelector('[data-testid="stMetricLabel"]');
                const valueEl = stMetric.querySelector('[data-testid="stMetricValue"]');
                const deltaEl = stMetric.querySelector('[data-testid="stMetricDelta"]');
                const label = labelEl?.textContent || '';
                const value = valueEl?.textContent || '';
                const delta = deltaEl?.textContent || '';
                return 'Métrica ' + label + ': ' + value + (delta ? ', cambio ' + delta : '');
            }}
            
            // ===== TABLAS =====
            if (tagName === 'TABLE') {{
                const headers = Array.from(element.querySelectorAll('th')).slice(0, 5).map(th => th.textContent?.trim()).join(', ');
                const rowCount = element.querySelectorAll('tbody tr').length;
                return 'Tabla con ' + rowCount + ' filas. Columnas: ' + headers;
            }}
            ...
            if (tagName === 'TD' || tagName === 'TH') {{
                ...
                if (text) {{
                    ...
                    const header = (headerCell && headerCell !== element) ? headerCell.textContent?.trim() + ': ' : '';
                    return header + text;
                }}
            }}
            
            // ===== ALERTAS de Streamlit =====
            const stAlert = element.closest('[data-testid="stAlert"]');
            if (stAlert) {{
                return 'Alerta: ' + stAlert.textContent?.trim();
            }}
            
            // ===== PÁRRAFOS y SPANS con texto =====
            if (tagName === 'P' || tagName === 'SPAN' || tagName === 'DIV' || tagName === 'LABEL') {{
                const text = element.textContent?.trim();
                // Solo leer si tiene texto significativo y no es muy largo
                if (text && text.length >= 3 && text.length <= 300) {{
                    // Verificar que no sea solo espacios o caracteres especiales
                    if (/[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ]/.test(text)) {{
                        return text;
                    }}
                }}
            }}
            ...
            if (tagName === 'LI') {{
                const text = element.textContent?.trim();
                if (text) return 'Elemento de lista: ' + text;
            }}
        ...
        function speak(text) {{
            ...
            const now = Date.now();
            // Evitar repetir el mismo texto muy rápido
            if (text === lastSpokenText && (now - lastSpokenTime) < COOLDOWN) {{
                return;
            }}
            ...
            // Pequeña pausa para asegurar que cancel() se procesó
            setTimeout(() => {{
                ...
                if (voices.length > 0) {{
                    ...
                    const voice = voices.find(v => v.lang === '{voz}') ||
                                 voices.find(v => v.lang.startsWith(langPrefix + '-')) ||
                                 voices.find(v => v.lang.startsWith(langPrefix)) ||
                                 voices.find(v => v.default);
                    if (voice) {{
                ...
                utterance.onerror = (e) => console.error('[TTS Hover] Error:', e);
        ...
        // Función de limpieza para cuando se reinicialice
        window.ttsHoverCleanup = function() {{
        ...
        // Indicador visual de que TTS hover está activo
        console.log('[TTS Hover] ✅ Inicializado correctamente - pasa el cursor sobre elementos para escucharlos');
        
        // Precargar voces (necesario en algunos navegadores)
        if (synth.getVoices().length === 0) {{
            synth.addEventListener('voiceschanged', () => {{
                console.log('[TTS Hover] Voces cargadas:', synth.getVoices().length);
            }});
        }}
    }})();
    </script>
...
def aplicar_accesibilidad():
    ...
    if st.session_state.get("a11y_tts_hover", False):
        _inject_tts_hover()
...
def _mostrar_contenido_panel_accesibilidad():
    ...
    if tts_activo:
        # Opción de TTS hover (leer al pasar el cursor)
        tts_hover = st.checkbox(
            "🖱️ Leer al pasar el cursor",
            value=st.session_state.get("a11y_tts_hover", False),
            key="a11y_tts_hover_cb",
            help="Lee automáticamente el contenido cuando pasas el cursor por encima. Ideal para personas con discapacidad visual.",
            on_change=_on_checkbox_change,
            args=("a11y_tts_hover",)
        )

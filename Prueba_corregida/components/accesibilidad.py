    ...
    (function() {{
        ...
        function getText(element) {{
            ...
            if (tag === 'IMG') {{
                ...
                const isLeftSide = imgCenter < pageWidth * 0.5;
                
                // PRIMERO: Verificar si ya tiene aria-label (agregado por script automático)
                const ariaLabel = element.getAttribute('aria-label');
                if (ariaLabel && ariaLabel.length > 10 && ariaLabel !== 'Gráfico o imagen') {{
                    return ariaLabel;
                }}
                
                // MÉTODO 1: Buscar TODOS los h3 (subheaders) ARRIBA de la imagen - PRIORITARIO
                ...
                for (let h3 of allH3) {{
                    ...
                    if (h3Rect.bottom < imgRect.top && h3Rect.bottom > 0) {{
                        const distance = imgRect.top - h3Rect.bottom;
                        // Para Pareto y otras gráficas puede haber más distancia, aumentar límite a 2000px
                        if (distance < closestDistance && distance < 2000) {{ 
                ...
                if (closestH3) {{
                    let headerText = closestH3.textContent ? closestH3.textContent.trim() : '';
                    // Limpiar emojis y espacios múltiples
                    headerText = headerText.replace(/^[📊🎯📈📝📦🔍]+/g, '').trim();
                    headerText = headerText.replace(/\s+/g, ' ').trim(); // Normalizar espacios múltiples
                    if (headerText && headerText.length > 0) {{
                        // Usar el h3 SOLO si está realmente en la misma columna
                        const h3Rect = closestH3.getBoundingClientRect();
                        const h3Center = h3Rect.left + h3Rect.width / 2;
                        const imgCenterPos = imgRect.left + imgRect.width / 2;
                        const horizontalDistance = Math.abs(h3Center - imgCenterPos);
                        
                        // Verificar contenido del h3 para gráficas del dashboard
                        const headerLower = headerText.toLowerCase();
                        const isDashboardAsistencia = headerLower.includes('análisis') && headerLower.includes('asistencia');
                        const isDashboardTendencia = headerLower.includes('tendencia') && headerLower.includes('unidades');
                        
                        // Para gráficas del dashboard, verificar columna pero más permisivo
                        if (isDashboardAsistencia || isDashboardTendencia) {{
                            // Deben estar en el mismo lado exacto (izquierda o derecha)
                            if ((h3Center < pageWidth * 0.5 && imgCenterPos < pageWidth * 0.5) ||
                                (h3Center >= pageWidth * 0.5 && imgCenterPos >= pageWidth * 0.5)) {{
                                // Y cerca horizontalmente (máximo 50% del ancho - muy permisivo)
                                if (horizontalDistance < pageWidth * 0.5) {{
                                    return construirDescripcionGrafico(headerText);
                                }}
                            }}
                            // Si no pasa la verificación de columna pero el h3 tiene el texto correcto, usarlo igual
                            // Esto asegura que se detecte aunque la columna no coincida perfectamente
                            return construirDescripcionGrafico(headerText);
                        }} else {{
                            // Para otras gráficas (como Pareto), usar directamente sin verificar columna
                            return construirDescripcionGrafico(headerText);
                        }}
                    }}
                }}
                
                // MÉTODO 2: Identificar por contexto de página usando posición y texto
                const pageTextLower = pageText.toLowerCase();
                
                // Distribución de Calificaciones - siempre arriba
                if ((pageTextLower.includes('distribución') && pageTextLower.includes('calificaciones')) && imgPosition < 0.7) {{
                    return construirDescripcionGrafico('Distribución de Calificaciones');
                }}
                
                // Análisis de Asistencia - lado izquierdo, parte inferior (solo si no hay h3)
                if (pageTextLower.includes('análisis') && pageTextLower.includes('asistencia')) {{
                    // Si está en parte inferior Y está a la izquierda
                    if (imgPosition > 0.25 && isLeftSide && imgCenter < pageWidth * 0.55) {{
                        return construirDescripcionGrafico('Análisis de Asistencia');
                    }}
                }}
                
                // Tendencia por Unidades - lado derecho, parte inferior (solo si no hay h3)
                if (pageTextLower.includes('tendencia') && pageTextLower.includes('unidades')) {{
                    // Si está en parte inferior Y está REALMENTE a la derecha
                    // Y NO contiene palabras de Análisis de Asistencia
                    if (imgPosition > 0.25 && !isLeftSide && imgCenter >= pageWidth * 0.52 && 
                        !pageTextLower.includes('análisis de asistencia')) {{
        ...
        function etiquetarGraficas() {{
            ...
            for (let img of allImages) {{
                if (img.width > 200 && img.height > 150 && !img.closest('[data-testid="stSidebar"]')) {{
                    ...
                    const imgCenter = imgRect.left + imgRect.width / 2;
                    const imgPosition = imgRect.top / win.innerHeight;
                    const pageWidth = win.innerWidth;
                    const isLeftSide = imgCenter < pageWidth * 0.5;
                    
                    // Buscar h3 más cercano arriba - CON VERIFICACIÓN DE COLUMNA
                    let closestH3 = null;
                    let closestDistance = Infinity;
                    for (let h3 of allH3) {{
                        if (h3.closest('[data-testid="stSidebar"]')) continue;
                        const h3Rect = h3.getBoundingClientRect();
                        // Buscar h3 que esté arriba de la imagen, con límite de distancia
                        if (h3Rect.bottom < imgRect.top && h3Rect.bottom > 0) {{
                            const distance = imgRect.top - h3Rect.bottom;
                            if (distance < closestDistance && distance < 2000) {{ // Limitar distancia a 2000px para capturar gráficas más lejanas como Pareto
                                // Verificar que estén en la misma columna
                                const h3Center = h3Rect.left + h3Rect.width / 2;
                                const imgCenterPos = imgRect.left + imgRect.width / 2;
                                const horizontalDistance = Math.abs(h3Center - imgCenterPos);
                                
                                // Obtener texto del h3 para verificar si es del dashboard
                                let h3Text = h3.textContent ? h3.textContent.trim().toLowerCase() : '';
                                h3Text = h3Text.replace(/^[📊🎯📈📝📦🔍]+/g, '').trim();
                                const isDashboardGraph = (h3Text.includes('análisis') && h3Text.includes('asistencia')) ||
                                                       (h3Text.includes('tendencia') && h3Text.includes('unidades'));
                                
                                // Para gráficas del dashboard, verificar columna pero más permisivo
                                if (isDashboardGraph) {{
                                    if ((h3Center < pageWidth * 0.5 && imgCenterPos < pageWidth * 0.5) ||
                                        (h3Center >= pageWidth * 0.5 && imgCenterPos >= pageWidth * 0.5)) {{
                                        if (horizontalDistance < pageWidth * 0.5) {{ // Más permisivo (50%)
                                            closestDistance = distance;
                                            closestH3 = h3;
                                        }}
                                    }}
                                    // Si no pasa la verificación pero el h3 tiene el texto correcto, usarlo igual
                                    if (!closestH3 || distance < closestDistance) {{
                                        closestDistance = distance;
                                        closestH3 = h3;
                                    }}
                                }} else {{
                                    // Para otras gráficas (como Pareto), NO verificar columna
                                    closestDistance = distance;
                                    closestH3 = h3;
                                }}
                            }}
                    ...
                    if (closestH3) {{
                        ...
                        headerText = headerText.replace(/\s+/g, ' ').trim(); // Normalizar espacios múltiples
                        if (headerText && headerText.length > 0) {{
                            // Verificar si es gráfica del dashboard
                            const headerLower = headerText.toLowerCase();
                            const isDashboardGraph = (headerLower.includes('análisis') && headerLower.includes('asistencia')) ||
                                                   (headerLower.includes('tendencia') && headerLower.includes('unidades'));
                            ...
                            if (isDashboardGraph) {{
                                ...
                                // Para otras gráficas (como Pareto), usar directamente
                                img.setAttribute('aria-label', construirDescripcion(headerText));
                                continue;
                            }}
                        }}
                    ...
                    // Fallback por contexto de página - MÉTODO PRINCIPAL (igual que Distribución)
                    const pageTextLower = pageText.toLowerCase();
                    
                    // Si no encontró h3, usar contexto de página con posición
                    if (!img.getAttribute('aria-label') || img.getAttribute('aria-label').length < 10) {{
                        // 1. Distribución de Calificaciones - arriba
                        if (pageTextLower.includes('distribución') && pageTextLower.includes('calificaciones') && imgPosition < 0.7) {{
                            img.setAttribute('aria-label', construirDescripcion('Distribución de Calificaciones'));
                        }}
                        // 2. Análisis de Asistencia - lado izquierdo, parte inferior (solo si no hay h3)
                        else if (pageTextLower.includes('análisis') && pageTextLower.includes('asistencia')) {{
                            // Si está en parte inferior Y está a la izquierda
                            if (imgPosition > 0.25 && isLeftSide && imgCenter < pageWidth * 0.55) {{
                                img.setAttribute('aria-label', construirDescripcion('Análisis de Asistencia'));
                            }}
                        }}
                        // 3. Tendencia por Unidades - lado derecho, parte inferior (solo si no hay h3)
                        else if (pageTextLower.includes('tendencia') && pageTextLower.includes('unidades')) {{
                            // Si está en parte inferior Y está REALMENTE a la derecha
                            // Y NO contiene palabras de Análisis de Asistencia
                            if (imgPosition > 0.25 && !isLeftSide && imgCenter >= pageWidth * 0.52 && 
                                !pageTextLower.includes('análisis de asistencia')) {{
